...
angular.module('starter.goodsList.service', [])
    .factory('goodsListService', [
        '$http',
        '$q',
        'globalConst',
        function ($http, $q, globalConst) {
            return {
                ...
                crossGetData: function (message) {
                    ...
                    var url = globalConst.SERVER_PATH + "/WXPlatformServlet?method=mobileUserUndoTaskList&message=" + message + "&loginName=" + $window.localStorage['loginName'] + "&platform=" + globalConst.PLATFORM + "&module=bill&callback=JSON_CALLBACK";
                ...
                test : function () {
                    ...
                    /* 创建一个 延迟对象 */
                    var deferred = $q.defer();
                    /* 改变数据请求后的状态值 -- resolve()方法 从unfulfilled到fulfilled */
                    /*deferred.resolve(dataTest);*/
                    /* 改变数据请求后的状态值 -- reject()方法 从unfulfilled到failed */
                    deferred.reject('我是失败后返回的reason');
                    /* 返回一个promise对象 */
                    return deferred.promise;
                }
            }
